...
const TableOfContents = () => {
  ...
  return (
    <>
      ...
      <nav className={`${styles.tocFloating} fixed left-4 lg:left-8 top-1/2 -translate-y-1/2 z-30 flex-col gap-4`}>
        {sectionKeys.map((key) => (
          <button key={key} onClick={() => scrollToSection(key)} className="group flex items-center gap-3 transition-all duration-300">
            <div
              className={`rounded-full transition-all duration-300 ${activeSection === key ? "w-3 h-3 bg-gold shadow-glow" : "w-2 h-2 bg-navy/30 group-hover:bg-burgundy/60"}`}
            />
            <span
