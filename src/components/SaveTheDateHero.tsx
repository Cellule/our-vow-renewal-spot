  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="font-script text-6xl md:text-8xl lg:text-9xl text-cream mb-4 drop-shadow-lg">{t("hero.saveDate")}</h1>
          <div className="w-32 h-px bg-gradient-gold mx-auto mb-6"></div>
          <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl text-champagne mb-6">{t("hero.renewingVows")}</h2>
        </div>

        <div className="bg-cream/10 backdrop-blur-md rounded-2xl p-8 md:p-12 border border-cream/20 shadow-elegant mb-8">
          <div className="grid md:grid-cols-2 gap-8 text-cream">
            <div>
              <h3 className="font-serif text-xl md:text-2xl mb-4 text-gold">{t("hero.couple")}</h3>
              <p className="font-sans text-lg md:text-xl font-medium whitespace-pre-line">{t("hero.coupleNames")}</p>
            </div>
            <div>
              <h3 className="font-serif text-xl md:text-2xl mb-4 text-gold">{t("hero.dates")}</h3>
              <p className="font-sans text-lg md:text-xl font-medium">{isWeekend ? t("hero.datesValueWeekend") : t("hero.datesValue")}</p>
            </div>
          </div>

          <div className="mt-8 pt-8 border-t border-cream/20">
            <h3 className="font-serif text-xl md:text-2xl mb-4 text-gold">{t("hero.location")}</h3>
            <p className="font-sans text-lg md:text-xl font-medium text-cream whitespace-pre-line mb-3">{t("hero.locationValue")}</p>
            <a
          ...
          <div className="mt-8 pt-8 border-t border-cream/20">
            <p className="font-sans text-sm md:text-base text-champagne mb-6">{t("hero.formalInvitation")}</p>
            <a
