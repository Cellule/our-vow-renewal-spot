...
function AddToCalendar() {
  ...
  const addToGoogleCalendar = () => {
    const googleCalendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(eventDetails.title)}&details=${encodeURIComponent(eventDetails.description)}&location=${encodeURIComponent(
      eventDetails.location.replace("\n", ", "),
    )}&dates=${eventDetails.startTime.replace(/[-:]/g, "")}/${eventDetails.endTime.replace(/[-:]/g, "")}`;
    window.open(googleCalendarUrl, "_blank");
  };

  const addToOutlook = () => {
    const outlookUrl = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(eventDetails.title)}&body=${encodeURIComponent(
      eventDetails.description,
    )}&location=${encodeURIComponent(eventDetails.location)}&startdt=${eventDetails.startTime}&enddt=${eventDetails.endTime}`;
  ...
  const downloadICSFile = () => {
    ...
    const start: [number, number, number, number, number] = [startDate.getFullYear(), startDate.getMonth() + 1, startDate.getDate(), startDate.getHours(), startDate.getMinutes()];

    const end: [number, number, number, number, number] = [endDate.getFullYear(), endDate.getMonth() + 1, endDate.getDate(), endDate.getHours(), endDate.getMinutes()];
  ...
  return (
    <div className="bg-gradient-elegant rounded-2xl p-12 text-center shadow-elegant">
      <div className="text-cream">
        ...
        <h3 className="font-serif text-4xl mb-6 text-cream">{t("contact.addToCalendar")}</h3>
